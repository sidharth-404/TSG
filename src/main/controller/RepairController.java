...
public class RepairController implements HttpHandler {
    ...
    public void handlePostRepairRequest(HttpExchange exchange) throws IOException {
        // Implement logic to create a book in the data layer and send response

        String requestBody = Utils.convertInputStreamToString(exchange.getRequestBody());
        JSONObject json = new JSONObject(requestBody);
        int bookId = json.getInt("bookId");
        String damageType = json.getString("damageType");
        final String repairStatus="pending";
        LocalDate repairStartDate=LocalDate.now();

        try {
            // Implement logic to insert new book into the database
            service.createRepairBook(bookId,damageType,repairStatus,repairStartDate);
            String response = "Book repair created successfully";
            sendResponse(exchange, 201, response);
        } catch (SQLException e) {
            e.printStackTrace();
            String errorMessage = "Error creating book repair in the database";
            sendResponse(exchange, 400, errorMessage);
        } catch (SqlHandleException e) {
            sendResponse(exchange, 400, e.getMessage());
